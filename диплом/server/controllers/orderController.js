...
class OrderController {
    ...
    async create(req, res, next, type) {
        try {
            const {first_name, second_name, delivery_type = 'courier', city, address, comment = null} = req.body
            // данные для создания заказа
            if (!first_name) throw new Error('Не указано имя покупателя')
            if (!second_name) throw new Error('Не указана фамилия покупателя')
            if (!city) throw new Error('Не указан город доставки')
            ...
            const order = await OrderModel.create({
                first_name, second_name, delivery_type, city, address, comment, items, userId
            })
